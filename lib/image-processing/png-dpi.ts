...
export async function embedPngDpi(pngBlob: Blob, dpi: number = 300): Promise<Blob> {
  ...
  // Construct pHYs chunk (9 bytes payload + 4 bytes length + 4 bytes type + 4 bytes CRC)
  ...
  // Combine Type + Payload for CRC calculation
  ...
  // Full pHYs Chunk Array (17 bytes)
  const physChunk = new Uint8Array(17);
  const physView = new DataView(physChunk.buffer);
  physView.setUint32(0, 9, false); // Length = 9
  physChunk.set(chunkType, 4); // Chunk Type
  physChunk.set(physPayload, 8); // Payload
  physView.setUint32(13, crc, false); // CRC
